"""PermissionEngine: optional hardline floor for full-access mode.

The hardline layer denies a curated set of unrecoverable / system-level
shell commands even in full-access mode (default-on; opt out with
``enable_hardline=False``). This module is dedicated to that floor —
its bypass-resistance against wrappers, quoting, splits, and shell
