...
# ---------------------------------------------------------------------------
# Sensitive-write preset rule (permission-hardening-plan §7, PR 5)
#
# The rule is mode-scoped to ``workspace-write`` and produces ASK — not DENY —
# so installers and devops scripts can proceed with operator confirmation.
# ``full-access`` deliberately does NOT carry this rule (literal full access),
# and ``plan`` mode already denies all shell commands anyway.
# ---------------------------------------------------------------------------
