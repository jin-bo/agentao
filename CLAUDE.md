...
**The `:memory:` backing is the one that needs locking.** A file-backed `SQLiteMemoryStore` opens a private connection per statement; the transient one keeps a single shared connection, because closing it would discard the database. It connects `check_same_thread=False` and serialises each `_connect` scope with an `RLock` — both halves are load-bearing, and the lock is what stops two threads inside one connection's implicit transaction from committing each other's half-written work. Tool calls already ran on the executor's worker threads when a batch held more than one; a background sub-agent's `save_memory` now reaches the *parent's* store from the sub-agent's thread unconditionally.
