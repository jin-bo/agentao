...
- `/clear` — saves current session, clears conversation + **all memories**, starts a new one. Shares one implementation with `/memory clear`, in two layers since #235: what a wipe *does* is `MemoryManager.wipe_all()` (`agentao/memory/manager.py`, so an embedded host performs the identical operation and reads the same `MemoryWipeResult`); what the CLI *says* about it is `cli/_utils.py::wipe_all_memories`, a shim that calls it and flattens the result to `(memories, summaries, not_cleared)`. Either way the change belongs in the shared layer, not in one of the two commands. There is deliberately **no** second copy of the wipe in the CLI: a manager with no `wipe_all`, one whose `wipe_all` raises, and one that answers with the wrong shape all come back as *both parts not cleared* rather than being re-wiped by hand — checking the result rather than `hasattr` is what keeps a `MagicMock` from reading as a successful wipe.
