...
def start_replay(
    ...
    try:
        ...
        agent._host_replay_sink = HostReplaySink(
...
def end_replay(agent: "Agentao") -> None:
    ...
    if adapter is not None:
        try:
            ...
            pass
    # Detach the host→replay observer so the next start_replay()
    # gets a fresh registration and we never double-write events.
    sink = getattr(agent, "_host_replay_sink", None)
    if sink is not None:
        try:
            sink.detach()
        except Exception:
