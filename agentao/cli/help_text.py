...
- `/thinking [minimal|low|medium|high|off]` - Show/set thinking depth (`reasoning_effort`; on the anthropic-messages wire `output_config.effort`, levels `low|medium|high|xhigh|max`; on openai-responses `reasoning.effort`); `off` restores the provider default. No auto-recovery: a model that rejects it fails until `off`
