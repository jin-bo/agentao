...
class AgentaoCLI:
    ...
    def _try_acp_explicit_route(self, user_input: str) -> bool:
        ...
        if not user_input:
            return False
        high_conf = bool(self._ACP_ROUTE_SHAPE_HIGH_CONF_RE.match(user_input))
        colon_shape = bool(self._ACP_ROUTE_SHAPE_COLON_RE.match(user_input))
        ...
        mtime_changed = (
            ...
            and disk_mtime != self._acp_config_mtime
        )
        needs_load = (
            self._acp_manager is None
            or not self._acp_manager.server_names
            or mtime_changed
        )
        if needs_load:
            try:
                from ..acp_client import ACPManager
                self._acp_manager = ACPManager.from_project()
                # Successful (re)load — cache the mtime we reloaded
                # against and rearm the one-shot error so a later config
                # regression prints a fresh message.
                self._acp_config_mtime = disk_mtime
                self._acp_load_error_shown = False
            except Exception as exc:
                if not self._acp_load_error_shown:
                    self._acp_load_error_shown = True
                    console.print(
                ...
                if high_conf:
                    # User clearly signalled delegation — do not silently
                    # re-route to the main agent.
                    return True
                # Ambiguous colon form — let ordinary prose through.
                return False

        mgr = self._acp_manager
        if mgr is None:
            return False
        names = mgr.server_names
        if not names:
            # No servers configured yet — nothing to route to.  Fall
            # through so the main agent handles the message; the next
            # explicit-route attempt will re-read the config.
            return False
        ...
        run_acp_prompt_inline(self, route.server, route.task)
        return True
    ...
    def _run_loop(self):
        ...
        while True:
            try:
                ...
                if self._try_acp_explicit_route(input_text):
                    self._flush_acp_inbox()
                    continue
