"""Translate :class:`HarnessEvent` payloads into replay JSONL events.

The embedded harness contract gives hosts a typed event stream
...
A :class:`HarnessReplaySink` takes a :class:`ReplayRecorder` and a
:class:`EventStream` and either:

- registers as an *observer* on the stream so every published harness
  event also lands in the replay file (push model), or
- accepts ad-hoc :class:`HarnessEvent` payloads and writes them
...
if TYPE_CHECKING:
    ...
    from .models import HarnessEvent  # noqa: F401
...
def harness_event_to_replay_kind(event: BaseModel) -> Optional[str]:
    """Return the replay-event ``kind`` for a public harness event.
...
def harness_event_to_replay_payload(event: BaseModel) -> dict[str, Any]:
    """Return the replay-payload JSON dict for a public harness event.
...
class HarnessReplaySink:
    ...
    - Pass ``stream=`` to auto-register as a synchronous observer on
      that stream so every published harness event is also recorded;
    ...
    swallowed — the harness contract requires the runtime to keep
    running even when audit storage is broken.
    """
    ...
    def attach_stream(self, stream: "EventStream") -> None:
        ...
        if self._stream is not None:
            raise RuntimeError(
                "HarnessReplaySink: stream already attached; call "
    ...
    def record(self, event: BaseModel) -> bool:
        """Write a harness event into the replay JSONL.
        ...
        if self._recorder is None:
            return False
        kind = harness_event_to_replay_kind(event)
        if kind is None:
            return False
        try:
            payload = harness_event_to_replay_payload(event)
            ...
            _logger.warning(
                "HarnessReplaySink: failed to record %s: %s",
                type(event).__name__,
                exc,
            )
            return False
...
def replay_payload_to_harness_event(
    kind: str, payload: dict[str, Any]
) -> Optional[BaseModel]:
    """Reverse projection: rebuild the public Pydantic model from JSONL.

    Used by tests / replay readers that want to rehydrate a harness
    event from disk. Returns ``None`` for kinds that are not on the
    v1.2 harness surface so callers can safely route mixed-kind reads
    through this helper.
    ...
    """
    # Lazy import: keeps the harness public surface independent of the
    # replay subpackage's import cost when reverse projection isn't used.
