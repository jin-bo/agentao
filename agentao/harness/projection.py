"""Projection / redaction helpers from runtime state to public harness models.

The harness contract treats public payload shapes as the compatibility
...
def project_matched_rule(rule: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    decision (see ``docs/api/harness.md``).
    """
    if rule is None:
        return None
    safe: Dict[str, Any] = {}
...
def _safe_publish(stream: EventStream, event: BaseModel) -> None:
    ...
    try:
        ...
        _logger.warning("harness projection invalid event: %s", exc)
    except Exception:
        pass
...
class HarnessPermissionEmitter:
