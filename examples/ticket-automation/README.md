...
- **Deny-by-rule posture** — `workspace-write` plus `deny` rules for `write_file` / `replace` / `run_shell_command` / `save_memory`, so the agent never touches the disk (`save_memory` is on the list because it writes `.agentao/memory.db`). Read-only mode would also deny this blueprint's own `draft_reply` / `send_reply`: it blocks every tool that is not read-only, before any rule is consulted.
