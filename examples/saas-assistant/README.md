...
- **Deny-by-rule posture** — `workspace-write` plus `deny` rules for `write_file` / `replace` / `run_shell_command`. Read-only mode would also deny `create_task`: it blocks every tool that is not read-only, before any rule is consulted.
