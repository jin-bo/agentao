...
一款功能强大的 CLI 对话智能体，支持工具调用与技能扩展。基于 Python 构建，兼容任意 OpenAI 兼容接口。
...
- 函数调用驱动工具执行
- 智能工具选择与调度
- **上下文窗口管理** — 自动滑动窗口压缩，通过 LLM 摘要保留关键信息
- **动态记忆召回** — Agentic RAG（无需向量数据库），每次对话自动关联相关记忆
...
- **工具确认机制** — Shell、Web 及破坏性操作需用户确认后执行
- **当前日期注入** — 系统提示词自动包含当前日期与时间
- **多行粘贴支持** — 粘贴多行文本时整体进入输入缓冲区（prompt_toolkit 原生支持，无定时 hack）；Alt+Enter 插入换行，Enter 提交
- **斜杠命令 Tab 补全** — 输入 `/` 后按 Tab 弹出补全菜单
- **可靠性原则** — 系统提示词在每轮对话中强制要求"读取后再断言"、报告差异、区分事实与推断
- **操作规范** — 系统提示词包含语气与风格规则、Shell 命令效率模式（临时文件重定向）、工具使用规则（并行调用、非交互式参数、尊重取消）及安全规范（先解释后执行）
...
每次响应前，Agentao 自动识别并注入与当前问题相关的记忆——无需向量数据库：
...
这意味着你保存的重要上下文（偏好、事实、项目细节）会在相关时自动浮现，无需主动询问。
...
如果当前工作目录中存在 `AGENTAO.md`，Agentao 会自动加载项目专属指令并注入系统提示词。你可以在其中定义：
...
项目指令注入在系统提示词顶部，优先级高于内置智能体指令。一个好的 `AGENTAO.md` 应包含代码风格规范、测试命令，以及可靠性规则（如要求智能体在声明代码库事实时引用文件和行号）。
