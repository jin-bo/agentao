...
| 读者 | 看哪里 | 生产站点 |
|---|---|---|
| **CLI 用户** — 在终端里用 `agentao` | [`developer-guide/zh/cli/`](developer-guide/zh/cli/)（12 章：slash 命令、plan 模式、记忆、回放…） | [agentao.cn/zh/cli/](https://agentao.cn/zh/cli/) |
| **嵌入开发者** — 把 Agentao 嵌进自己的应用 | [`developer-guide/zh/`](developer-guide/zh/)（第 1–7 部分 + 附录） | [agentao.cn/zh/](https://agentao.cn/zh/) |
...
在 `developer-guide/` 下执行 `npx vitepress dev` 可本地预览完整指南。

需要某个主题的详细参考时，权威文档在 `docs/`：
