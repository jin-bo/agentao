...
- 第一次使用 Agentao：[快速开始](#快速开始) → [最小可运行配置](#最小可运行配置) → [使用方法](#使用方法)
...
| 自动化 | 非交互模式、SDK、ACP、子智能体 | [使用方法](#使用方法) |
...
| 目标 | 建议阅读路径 | 可以马上尝试 |
|------|-------------|-------------|
| 第一次成功跑起来 | [快速开始](#快速开始) → [最小可运行配置](#最小可运行配置) | `agentao -p "Reply with the single word: OK"` |
