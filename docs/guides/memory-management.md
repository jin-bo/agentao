...
| 用户库 | `<home>/.agentao/memory.db` | 跨项目用户级持久记忆 |
...
| 结晶器提案 | 同上 | 子 agent 的 `role: "user"` 消息是父模型写的任务 prompt；落进父库的 `memory_review_queue` 后可被 `/memory review approve` 提升成用户从未说过的记忆，而该表**没有任何清除路径** |
| 读取（`<memory-stable>` / 召回） | **无** | transient store 是空的：子 agent 不读任何记忆，改由 `parent_context` 交代背景 |
