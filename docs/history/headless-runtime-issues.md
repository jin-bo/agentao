...
`ACPManager.get_status() -> List[Dict[str, Any]]` 已存在（`manager.py:1631`），返回 CLI-friendly 字典列表，字段包括 `name` / `state` / `pid` / `last_error` / `last_activity` / `inbox_pending` / `interactions_pending` / `description` / `stderr_lines`。`docs/guides/acp-embedding.md` 也已经把 dict 列表写成了 embedding contract。
...
- 新文档：`docs/guides/headless-runtime.md`
...
- `docs/guides/headless-runtime.md` — 同步更新 snapshot v2
...
- `docs/guides/headless-runtime.md` — 同步更新
...
- `docs/guides/headless-runtime.md` — 终版（含 lifecycle、regression suite 入口）
...
- `docs/guides/headless-runtime.md` 随每周变动渐进更新，不一次写完
- 本文件位置 `docs/history/headless-runtime-issues.md` 保留至评审结束；签字后建议按 `docs/implementation/acp-client-project-servers/issues/` 先例展开为目录 + 每 issue 一文件 + `README.md` 索引
