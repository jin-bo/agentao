...
- `docs/history/implementation/plugin-system-mvp/phase-6-session-tool-hooks-and-cli.md` —— 在支持事件列表中追加新增两个事件。
- `docs/reference/configuration.md` —— 若枚举了 hook 事件清单，同步更新。
...
- `_stop_reentry_cap: int` —— **chat loop 的构造参数，硬编码默认值为 `3`**。Phase B **不**从 `.agentao/settings.json` 读取这个值。该文件目前只有两个读者（`embedding/factory.py::_load_settings`、`plan/controller.py::_load_settings`，见 `docs/reference/configuration.md` §3）；为一个真实 host 撞封顶之前没人会调的旋钮再加第三个 one-key reader，是过早暴露配置面。如果将来某 host 明确要求运行时调参，再把字段提升为 `stop_hook_reentry_max` settings key、走那时已有的统一 settings loader。
