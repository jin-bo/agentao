...
Parent phase: [Phase 5: UserPromptSubmit And Hook Core](../phase-5-user-prompt-submit-and-hook-core.md)
...
- per-event hook-type allowlist (`SUPPORTED_HOOK_TYPES_BY_EVENT`) — added by [`STOP_PRECOMPACT_HOOKS_PLAN.md`](../../stop-precompact-hooks-plan.md) §A1; rejects `prompt`-type rules under `Stop` / `PreCompact` at parse time
