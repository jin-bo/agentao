...
**实施计划（历史档）:** [`docs/history/implementation/embedded-harness-contract-implementation-plan.md`](../history/implementation/embedded-harness-contract-implementation-plan.md)
...
> > [Embedding vs. ACP](../design/embedding-vs-acp.zh.md)。
...
`SubagentLifecycleEvent(phase="failed")` 以前只有一个含义：子 Agent
**抛了异常**。现在它还会在子 Agent 正常返回、但**始终没给出答案**时
触发——空 turn、只有 reasoning 没有答案、被输出长度截断、doom-loop 被
拦停、LLM 调用失败，或者 turn 预算耗尽。把这些报成 `completed` 会让
这份契约陈述一件不真实的事，所以它们落到 `failed`。

**对按 `phase == "failed"` 分支的宿主来说，这是一次破坏性的语义变更。**
原本在这个 phase 上呼叫 on-call、开事故单或重试父 turn 的宿主，现在会
被普通的"没答出来"触发——而这类情况很常见，通常并不是事故。请改用
`error_type` 区分两者：

| `error_type` | 含义 |
|---|---|
| `"incomplete:<reason>"` | 子 Agent 返回了但没答出来。不是异常。 |
| 其他任意值（如 `"ValueError"`、`"TimeoutError"`） | 子 Agent 抛了异常，值是异常类名。 |
...
if isinstance(ev, SubagentLifecycleEvent) and ev.phase == "failed":
    if (ev.error_type or "").startswith("incomplete:"):
        metrics.non_answer(ev.error_type.split(":", 1)[1])   # 预期内
    else:
...
**或者让模型在轮次内等待。** `check_background_agent` 有一个可选参数
`wait_seconds`（默认 `0`，最多 1800）：一次调用会阻塞到子代理结束，并在同一轮
内返回结果。ACP 客户端只能由自己开启轮次，所以走这条路。取消这一轮——ACP 的
`session/cancel`、Ctrl+C、传给 `chat()` 的 token——会在半秒内结束等待，但子代理
继续运行；只有 `cancel_background_agent` 能停掉它。等待超时时会明说，并告诉
...
### `FileSystem.write_text` 必须原子地替换

**替换已有内容**的实现必须给调用方一次原子替换：读者要么看到旧内容、
要么看到新内容，绝不能看到被截断或写了一半的文件。Agentao 跑在一个它
控制不了的宿主进程里，所以"先截断再写"会留出一个窗口——一次 Ctrl+C、
一次 OOM kill 或一个 `kill` 就会毁掉用户的文件。这是对**你自己的**实现
提出的要求，不只是对默认实现。

`LocalFileSystem.write_text` 是参考做法，其中两个可观测行为对包装或审计
文件系统的宿主有影响：

- 它会在目标所在目录里暂存一个**同级临时文件**，命名为
  `.{name}.*.tmp`，然后 `os.replace` 换入。审计包装层、文件 watcher 和
  虚拟文件系统看到的是这一对 create/rename，而不是对目标路径的一次写。
- **只读目标会抛 `PermissionError`。** `os.replace` 只需要*目录*的写
  权限，所以原子写本来会悄悄覆盖一个 `chmod 444` 的文件——而旧的直写
  路径是会拒绝的。这次拒绝是显式且刻意的。

有两种情况仍走直写路径，因为它们都不可能破坏已有内容：`append=True`，
以及目标尚不存在。

范围说明：这关掉的是*进程死亡*窗口。它没有 fsync，所以掉电级别的持久性
仍归宿主负责。
...
[`docs/reference/replay-schema-policy.md`](replay-schema-policy.md)。
...
- **Turn 结果作为流式事件（push）。** 结果本身——模型到底答没答——已经
  ...
  **子 agent** 的 turn 结果*已经*在公共面上了——没答出来的子 agent 会发
  ...
  见[子 Agent 的 `failed` 有两种形态](#子-agent-的-failed-有两种形态)。
  但**主**循环自己的 turn 仍然没有公共事件，所以这类观察者只能退回到内部
  `Transport` 的 `TURN_END`（未投射，且有上文的 `schema_version` 警告）。这条 **尚未**记录在
  [PUBLIC_EVENT_PROMOTION_PLAN](../history/implementation/public-event-promotion-plan.md)
