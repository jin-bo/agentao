...
|---|---|---|---|
| Background sub-agent task state | `.agentao/background_tasks.json` | `agents/bg_store.py::BackgroundTaskStore` | Anchored to `working_directory`; in-memory only when no `persistence_dir`. Hand-edits will desync running threads. A record with `status: "failed"` carries `incomplete_reason` when the run finished without raising but never answered — its presence is what separates "crashed" (no `result`) from "stopped short" (`result` still present and worth reading). Absent on records written before 0.4.15. Since 0.4.24 a record with `status: "cancelled"` keeps the `result` and the counters the run produced when it was cancelled *while running*, and carries neither when it was cancelled before it started; `incomplete_reason` is `None` on both — the status already names the cause. **Not secret-scanned** — see the note below. |
...
| `{PROVIDER}_API_FORMAT` | no | `openai-completions` | The **wire protocol** spoken to that provider block's endpoint: `openai-completions` (Chat Completions), `anthropic-messages` (Anthropic's Messages API, over the official `anthropic` SDK, a core dependency) or `openai-responses` (OpenAI's Responses API, over the same `openai` SDK as Chat Completions; 0.5.3). It rides the provider prefix because it is a property of the endpoint, and it is **never inferred** from the base URL, the provider name or the model name: `LLM_PROVIDER=ANTHROPIC` pointing at an OpenAI-compatible gateway is a working configuration and stays on Chat Completions until you say otherwise. An unknown or not-yet-implemented value (`gemini-api`) **raises** at startup and lists the valid ones; empty / unset means the default. **A provider switch carries it:** `/provider` reads the target block's value and changes the wire along with the credentials — clearing what a model or endpoint change clears (thinking artifacts in history, the token anchor, the observed context limit, the capability latches, the explicit prompt-cache breakpoints) and keeping `LLM_EXTRA_BODY`, whose keys it names because they were written for the other protocol; a bad value on the target block leaves the session where it was. Sub-agents are built on the parent's current wire. On `anthropic-messages`: `{PROVIDER}_BASE_URL` is the API root (`https://api.anthropic.com` — a trailing `/v1` carried over from the compat endpoint is dropped, since the SDK appends `/v1/messages`); **`LLM_TEMPERATURE` and `/temperature` have no effect** — `anthropic` 1.6.0 has no `temperature` parameter on `messages.create` at all, so a gateway that does take one gets it through `LLM_EXTRA_BODY`; `max_tokens` is required by the protocol, so a call that names none sends `LLM_MAX_TOKENS` (default 65536), and the model's cap is taken from the provider's Models API (`GET /v1/models/{id}`, asked once per model before the first request) so a larger value is clamped rather than rejected; on an endpoint without that route (compatible gateways mostly answer 404, which changes nothing) a model that answers `max_tokens: N > M` has `M` adopted for the rest of the session at the cost of that one rejected request — set `LLM_MAX_TOKENS` to the model's cap to avoid it. The same lookup's `max_input_tokens` can narrow the context window (`AGENTAO_CONTEXT_TOKENS` / `max_context_tokens=` stays the ceiling; see host-api, *Two context windows*); extended thinking is turned on through `LLM_EXTRA_BODY` — `{"thinking": {"type": "adaptive"}, "output_config": {"effort": "high"}}` on current models (`claude-sonnet-5` rejects the older form with a 400), `{"thinking": {"type": "enabled", "budget_tokens": 8000}}` on older ones; a current model may return its thinking *text* empty beside the signature, so there is nothing for the CLI to show — and **`/thinking <level>` writes `output_config.effort`** here rather than `reasoning_effort`, which this protocol rejects (levels `low | medium | high | xhigh | max`, or those the Models API's `capabilities.effort` marks supported; anything else is refused, not stored; `off` clears it); `LLM_PROMPT_CACHE=anthropic` places the same three breakpoints natively. Signed thinking blocks are kept whole on the assistant message under `anthropic_thinking_blocks` (session files and replay included) and sent back verbatim; a model or endpoint switch, `/resume` and ACP `session/load` drop them. See `agentao/llm/_anthropic_messages.py` and `docs/design/llm-api-adapters.md` §6. On `openai-responses`: `{PROVIDER}_BASE_URL` is what Chat Completions takes (`https://api.openai.com/v1`; the SDK appends `/responses`); requests are **stateless** — `store: false`, no `previous_response_id`, the whole history every time, because agentao's history is what compaction, `/clear` and replay act on; `LLM_MAX_TOKENS` goes out as `max_output_tokens` (raised to the API's floor of 16) and `LLM_TEMPERATURE` is sent until a model rejects it, as on Chat Completions; `LLM_PROMPT_CACHE` places nothing here (the API caches on its own, and the cached part is reported in `cache_read_tokens`); tools are sent `strict: false`. A reasoning model's reasoning is kept across requests: every request asks for `reasoning.encrypted_content`, the items are kept whole on the assistant message under `openai_reasoning_items` (session files and replay included) and sent back ahead of their turn; a model or endpoint switch, `/resume` and ACP `session/load` drop them, and an endpoint that rejects the field is asked once more without it and from then on gets neither the field nor the items. **`/thinking <level>` writes `reasoning.effort`** here (`minimal | low | medium | high`; another word is passed through for the provider to validate; `off` clears it, and a `reasoning_effort` carried in from another wire with it); reasoning *summaries* — the only reasoning text this API shows — are off unless asked for, `LLM_EXTRA_BODY='{"reasoning": {"summary": "auto"}}'`. A tool call is kept in history as `call_id|fc_…`; after a switch to Chat Completions it goes out as the `call_id` alone, since the composite (83 characters) is longer than the 64 OpenAI allows there. See `agentao/llm/_openai_responses.py` and `docs/design/llm-api-adapters.md` appendix A. |
| `LLM_TEMPERATURE` | no | `0.2` | Range `0.0`–`2.0`. Malformed value **raises** at startup (`float()`). Not sent on the `anthropic-messages` wire — see `{PROVIDER}_API_FORMAT`. |
...
| `LLM_PROMPT_CACHE` | no | off | Explicit prompt-cache breakpoints over the ordinary Chat Completions wire. `anthropic`, or `off` / empty / unset for none. When on, each agent-turn request carries at most **3** `cache_control: {"type":"ephemeral"}` markers — on the system message, the last tool definition, and the end of stable history — leaving the 4th breakpoint slot free for the endpoint's automatic caching. **Opt-in by design:** agentao verified that the OpenAI SDK forwards the key unchanged (openai 2.24.0), but *not* that any given gateway honours it, so this is never inferred from your base URL or model name — you name it for an endpoint you have verified. An unknown value **raises** at startup (unlike `LLM_EXTRA_BODY`): a typo here would silently disable the one thing it was set to enable. Markers never enter history, session files or the replay record, and are dropped automatically when `/provider` changes the base URL (a new deployment the assertion no longer covers). Markers a host placed on its own messages count against the three and are never overwritten, so their `ttl` survives — and they constrain where agentao may add its own: Anthropic requires longer-lived entries to precede shorter-lived ones, so a site that would put a configured `1h` behind a caller's `5m`, or a configured `5m` ahead of a caller's `1h`, is skipped rather than re-timed. A `ttl` agentao does not recognise suppresses its own markers entirely rather than guessing a rank. **Three things to check on the endpoint you are verifying**, because agentao cannot: (1) it forwards `cache_control` rather than 400ing on it; (2) it accepts a `role: "tool"` message whose `content` is an array of text parts — legal OpenAI, but the conversation breakpoint lands on a tool result whenever the previous iteration ran tools, and some gateways accept only a plain string there; (3) its `usage.prompt_tokens` counts cached tokens *in* rather than net of cache reads. (3) matters because the Tier-1 token anchor reads that field as the cost of the persistent prefix: a gateway reporting Anthropic's cache-exclusive `input_tokens` under that name makes a cached 40k history read as a few hundred tokens, and compaction stops firing until the provider's own context-length error drops the turn into the overflow ladder. An anchor that collapses all the way to zero is dropped rather than trusted, but a merely implausible one is not detectable from a single response — this is a known limitation of stage 0b, not a solved problem. See `agentao/llm/_cache_control.py` and `docs/design/llm-api-adapters.md` §2.3. |
| `LLM_PROMPT_CACHE_TTL` | no | `5m` | Retention hint for the markers above: `5m` (the provider default, same as unset) or `1h`. Ignored when `LLM_PROMPT_CACHE` is off — *including* the validation, so an unknown value **raises** at startup only when the format is on, and is silently unread otherwise. |
...
- **Write mode.** `/skills disable` and `/skills enable` do a read-modify-write of **one name**, under a `filelock` on `skills_config.json.lock` (10s, the same protocol `skills/registry.py` uses) that spans the read *and* the write, then swap the file in via a temp file and `os.replace`. Every other disabled name, and every other key in the file, survives — including ones a second agentao process wrote after this one started; "already disabled" / "is not disabled" is answered from the file, not from the set this process read at construction. The write path parses **strictly**, unlike the load above: malformed JSON, a non-object top level, a non-array `disabled_skills`, a non-string entry inside it, or a decode error each **refuse the write** and return an error naming the path — the lenient view the loader falls back to would, written back, delete exactly the names the file holds. A lock timeout or a failed write does the same. On any of those the file, the in-memory set and the active skills are all left untouched, and nothing reports success. The lock binds agentao processes following this protocol; a text editor saving the file at the same moment is outside it.
...
| Key | Type | Notes |
|---|---|---|
| `disabled_skills` | string[] | Skill names to **exclude** from auto-discovery. Use `/skills disable <name>` from the CLI to manage. A disabled skill is also **refused at activation** — by the `activate_skill` tool, by `/skills activate`, and in any sub-agent, which inherits the set as it stands when the sub-agent starts (a disable made while one is running reaches the next sub-agent, not that one). The tool answers "Unknown skill" (to a caller trying to activate it, a disabled skill is not there); `/skills activate` names the state and the remedy. Re-enable it (`/skills enable <name>`) before activating. The list is **never reconciled against what a scan found**: a reload that cannot see the skill (unmounted share, directory renamed mid-session) leaves the name in place and writes nothing, so the skill comes back still disabled — a scan cannot tell "deleted" from "not discoverable right now", and since a disable is an activation gate, guessing wrong would silently re-arm the skill. Only `/skills disable` / `/skills enable` change the list; the cost is that a name can outlive the skill it disabled — `/skills` lists such a name under **Disabled, not found by the last scan**, and `/skills enable <name>` clears it whether or not the skill is present. |
...
- **Deletion is soft, so the file does not shrink.** `/memory delete`, `/memory clear` and `MemoryManager.wipe_all()` set `deleted_at`; the row keeps its `content` and stays in `memory.db`, and no read path returns it again. Only your own `DELETE` + `VACUUM`, or disposing of the file, removes the bytes — relevant if you are building an erasure guarantee on top of a wipe. Session summaries, by contrast, are a real `DELETE`.
...
- **Sub-agents write here too — and read nothing from here.** A sub-agent's `save_memory` writes through the **parent's** manager, so a memory it saves lands in the same stores, at the same scope, and through a host-injected manager if there is one (#260). Everything else about its memory is its own: since #234 it is built on a transient (`:memory:`) store, so its *session summaries* and its crystallized review proposals stay there and are discarded with it — they used to land in the project DB above, because the child's default opened the very same file. That store is also empty, which is the read side of the same decision: **a sub-agent gets no `<memory-stable>` block and no recall.** It is briefed by the parent's recent *messages* (`parent_context`), which carry no memories, so it can save a long-term memory it cannot read back. Rows a sub-agent wrote into the project DB before #234 carry no marker; `/clear` and `/memory clear` remove every session summary and every memory but reach no review item. (#234)
