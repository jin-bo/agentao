...
**对照件：** `review-method-rules.md`。

第 1–32 条来自 PowerShell 支持设计的四十二轮评审。那套设计文件已经退役（`powershell-support-lightweight.zh.md` 记录最终方案，
历史在 Git 里），但规则本身与那个题目无关 —— 每一条都是因为「没有它」而漏过了缺陷，按漏过的次数排序，对任何一次评审都成立，
所以单独留在这里。

**第 33 条来源不同** —— 出自 2026-09-20 对 `docs/design/` 全部 `Status:` 行的审计，不是 PowerShell 那几轮 —— 因此**附在末尾而不参与排序**：
一个案例不构成频次。本文件现在是三十三条；第 8 条适用于这句话本身。
...
33. **甲文档里写下的「推迟」，会被乙文档的实施推翻，而两者之间没有任何联系 —— 所以一个 PR 实现了某项能力时，
    要反向 grep 其余**所有**仍声称它未建的地方。** 2026-09-20 的状态审计发现 `agentao/cli/commands/goal.py:6`
    把 `codex-goal-mechanism-review.md` §11.1 引为接口规格，而那份文档在三处、且中英双份，都写着 §11
    「非已批准计划 / 未实现」。§11 早已**整节**随 0.4.13 发布 —— 就在文档起草的次日 —— 这个状态一路活过了
    0.4.14 到 0.5.3 的十八个版本。**代码自己的指针，把读者送到一份告诉他「这东西不存在」的文档上。**
    同一句话**还落进了源码**：`acp/session_set_mode.py` 的模块 docstring 在「发出 `current_mode_update`
    的那个函数」上方四十行处说它被推迟 —— 因为真正实现它的（ACP G4 PR-1）属于*另一份*设计，
    而 docstring 指向的 `deepchat-acp-patch-revision.md` 又不是被改的那一份。这是唯一会逃进代码的一类漂移，
    也正因如此才值得做一次「读任何一份文档都做不出来」的 grep：**两份文档各自说自己的事，都没有说错。**
    **两条推论。** 其一，*部分 restatus 比完全不做更糟*：`acp-server-conformance-review.md` 给 G3 挂了
    「已在 0.4.16 解决」的横幅，而同样已闭合的 G4（0.4.12）、G5（0.4.14）没有，于是这两条读起来像是**刻意仍开放**。
    给一条挂横幅，就要把其余的一并扫完。其二，*审计自己的算术同样是断言*（第 8 条）：本次修复的初稿把漂移写成
    「十五个月」——取自子代理报告里的数字，实际不到三个月；改正后又写成「十二个版本」，实际是十八个。
    两次都是先把数字写进**补丁本身**，之后才去数。
