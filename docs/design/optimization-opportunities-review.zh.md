...
> **部分被取代（2026-07-25）。** Tier 1–2 已随 v0.4.12 落地（见文末*实施状态*）。
> **Tier 3 已正式否决**——`refactor-audit-2026-07.md` §5 重新核查了三项，发现
> "维护成本高"的前提不被 12 个月的 churn 数据支持，且 T3.2 还会破坏嵌入宿主的
> 既有接口。没有新证据前不要重提。

**对照件：** `optimization-opportunities-review.md`。
**相关：**
- `refactor-audit-2026-07.md` —— 七月的后续审计 + 反向评审；否决本文的 Tier 3，
  并记录了还有哪些提案被否及其理由。
