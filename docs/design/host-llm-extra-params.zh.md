...
两处都只构建 `{model, messages, temperature?, tools?, tool_choice?, max_tokens|max_completion_tokens?}`，再传给 SDK——非流式经 `client.chat.completions.with_raw_response.create(**kwargs)`（`client.py:339`），流式经 `client.chat.completions.create(**kwargs)`（`client.py:613`）。（两个方法不同；实现者测试时必须 spy 各路径*正确*的那个——见 §9。）构造函数（`client.py:90-100`）只暴露 `api_key / base_url / model / temperature / max_tokens / log_file / logger`——**没有任何字段**承载额外请求参数。
...
| **CLI / 环境变量** | `discover_llm_kwargs()`（`factory.py:57`）将 `LLM_EXTRA_PARAMS` 读为 JSON **对象**，置于 `try/except` 内解析：JSON 非法 → `告警 + 跳过`。**还须拒绝合法但非对象的 JSON**——`LLM_EXTRA_PARAMS=[]` / `"x"` / `3` 解析得通却是无效配置；要求 `isinstance(parsed, dict)`，把非对象按畸形同等对待（告警 + 跳过），从而由**环境告警策略**治理 env 路径，而非在构造时抛出令人困惑的 `TypeError`（§3.1）。**注意：** 这是*有意比*既有 `LLM_TEMPERATURE` / `LLM_MAX_TOKENS` *更宽容*——后者直接调 `float()` / `int()`，遇畸形值会**抛错**（`factory.py:79-82`）；`build_from_environment` 今天只是在提供 `llm_client` 时整体跳过 discovery 才绕开（`factory.py:134`）。`try/except` 与 `isinstance` 检查都必须显式加上——两者都**不是**从某个既有容错继承来的。 |
...
- **日志（v1 显式改动——并非"免费"，必须脱敏）**：`_log_request`（`agentao/llm/_logging.py:23`）记录的是**固定字段集**——`model`、`temperature`、`max_tokens`、`messages`、`tools`——*而非*任意请求 kwargs，故直通参数**不会**自动出现。v1 增加一行记录合并后的非保留额外参数，但值必须**递归脱敏**：把键名匹配 `authorization` / `api[-_]?key` / `token` / `secret` / `password` / `cookie`（不区分大小写、任意嵌套深度）的值在记录前替换为 `***`——如 `Extra params: {'reasoning_effort': 'high', 'extra_headers': {'Authorization': '***'}}`。这是**必需而非装饰**：`extra_params` 显式支持 SDK 的 `extra_headers` / `extra_body` 逃生舱（§1），故 `extra_params={"extra_headers": {"Authorization": "Bearer …"}}` 否则会把一个真实凭据泄漏进 `agentao.log`——而这正是当前 logger 有意完全挡在 kwargs 路径之外的一类机密（`api_key` 在客户端上，从不进入被记录的 kwargs）。脱敏器是 `_logging.py` 里的一个小递归 helper；针对嵌套 `extra_headers.Authorization` 有测试（§9）。
...
- `extra_params` 合并进 `chat()` 与 `chat_stream()`——spy 各路径**正确**的 SDK 方法：`with_raw_response.create`（非流式，`client.py:339`）与 `create`（流式，`client.py:613`）；断言键存在。
...
- **环境容错（F3）：** `discover_llm_kwargs()` 解析合法 `LLM_EXTRA_PARAMS` JSON 对象；非法 JSON → 省略该键 + 告警，不抛错。
- **环境非对象（P2）：** `LLM_EXTRA_PARAMS=[]` / `"x"` / `3`（合法 JSON、非对象）→ 经环境策略省略该键 + 告警，**而非**构造时 `TypeError`。
- **日志脱敏（F5/P1）：** 当 `extra_params={"reasoning_effort":"high","extra_headers":{"Authorization":"Bearer x"}}` 时，`_log_request` 显示 `reasoning_effort` 但嵌套的 `Authorization` 值为 `***`；无 `extra_params` 时该行缺省。
...
| `agentao/llm/_logging.py` | 在 `_log_request` 中记录合并后的额外参数，敏感键的值**递归脱敏**（§7） |
| `agentao/agent.py` | 加 `extra_params` kwarg；穿入 `_build_llm_client`；**加入互斥守卫**（§4.1） |
| `agentao/embedding/factory.py` | 在 `try/except` + `isinstance(parsed, dict)` 中解析 `LLM_EXTRA_PARAMS` JSON（畸形**或**非对象 → 告警 + 跳过） |
