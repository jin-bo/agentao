...
**状态：** 已落地 2026-05-25 —— **发布于 0.4.8**，该版本号由 git 反查得出（`9698f6f`，
`git describe --contains` → `v0.4.8~18`），**而非取自 `CHANGELOG.md`：这个特性在
changelog 里根本没有任何条目。** 这是发布记录本身的洞，不是状态行缺失；此处只做记载、
不回填——changelog 是一个版本当时**说了什么**的记录，事后修改已发布版本的条目，会让其余
所有条目一并变得不可信。若只凭 changelog 审计「0.4.8 发了什么」，是找不到这一条的。
实现说明见文末。
