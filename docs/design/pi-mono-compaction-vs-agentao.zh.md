...
> **⚠️ 两条 P1 其后均已实施，并随 0.4.20 发布（2026-08-24）**——见下方 restatus 横幅。
> **其余部分仍仅为分析记录，除这两条外全文任何一条都未获授权实施。** §1 的分级是**分析结论的优先级排序**，不是工单，
> 也不是排期。引用那张表时请连同这一行一起引用——它防止下一个读者把排序读成迭代计划。

> **⚠️ Restatus（2026-09-19）——两条 P1 均已关闭。** 它们由
> `compaction-orchestration-plan.md`（六个 PR，#187–#192）承接，**随 0.4.20 发布**，就在本文锚点
> 的次日。`docs/design/README.md` 自该计划落地起就写明了这一点，本文没有，并且在此后十一个版本里
> 一直写着「未授权实施」。
>
> - **§8 —— `PreCompact` 现在可以否决。** hook 读
>   `hookSpecificOutput.compactionDecision`（`plugins/hooks/_dispatcher.py:336`）；宿主 controller
>   还可以直接替换摘要（`compaction/types.py:117` 的 `can_provide_summary`，在
>   `context_manager.py:1586` 生效）。hook 层先判、首个 cancel 胜出，然后才是 `compaction_controller=` 层。
> - **§9 —— 熔断器有复位路径了。** `ContextManager.reset_compaction_circuit()`
>   （`context_manager.py:833`，成功时于 `:1636` 调用），外加半开**探针**：手动 `/compact` 与
>   `api_overflow` 可穿过打开的熔断器运行，探针成功即关闭它（见 `compaction/coordinator.py`
>   的 `_PROBE_REASONS`）。
>
> 下方 §8 / §9 正文按当时对 `main@a996395` 的原始结论保留。P2/P3 各条**不受**本横幅影响，
> 仍未获授权。
...
**状态：** 分析，rev 6（2026-08-23）。**两条 P1 已随 0.4.20 发布**（2026-08-24，经
`compaction-orchestration-plan.md`）；其余各条仍未获授权。2026-09-19 restatus。
...
| 若实施，优先级 | 内容 | 章节 |
|---|---|---|
| **P1** —— ✅ **已于 0.4.20 关闭** | `PreCompact` 插件钩子是只读通知（`-> None`）。嵌入方能观察压缩，但**不能否决、也不能替换结果**。pi-mono 的 `session_before_compact` 两者都能做。 | §8 |
| **P1** —— ✅ **已于 0.4.20 关闭** | 摘要失败熔断器**没有复位路径**：一旦打开就在任何尝试之前短路，于是永远不可能成功，于是永远不复位。压缩——**包括手动 `/compact`**——在该 `ContextManager` 实例的余生里报废。 | §9 |
...
## 8. 可扩展性——P1 · ✅ **已于 0.4.20 关闭**

> 下文的缺口按原文成立（对 `main@a996395`），并**已于 0.4.20 关闭**：`PreCompact` hook 可经
> `hookSpecificOutput.compactionDecision` 否决（`plugins/hooks/_dispatcher.py:336`），
> `compaction_controller=` 还可以替换摘要（`compaction/types.py:117`）。凡不是显式 cancel 的
> 一律视为放行——**包括抛异常**——因为五个入口中有两个是溢出恢复阶梯。
...
## 9. 失败处理 · 「无复位路径」那条 P1 ✅ **已于 0.4.20 关闭**

> 熔断器现在是可恢复状态机：`reset_compaction_circuit()`（`context_manager.py:833`，成功时于
> `:1636` 调用），`/clear` 也会关闭它；手动 `/compact` 与 `api_overflow` 作为半开**探针**穿过
> 打开的熔断器运行，探针成功即关闭它。状态仍归 `ContextManager`，coordinator 只施加策略。
> 下方正文为当时的原始结论。
