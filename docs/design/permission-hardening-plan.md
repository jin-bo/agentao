...
- **`bashlex` supersedence of PR 5's regex.** **Open, tracked here as the only inherited follow-up.** PR 5's regex catches the common shapes (redirect, tee, cp/mv, sed -i) targeting `~`/`$HOME`-prefixed sensitive files. It cannot catch:
    ...
    A `bashlex`-based pass would close (1) and (3). **Correction (2026-09-19):** this sentence used to call that "the same approach the hardline shell-safety scanner uses for `rm -rf` indirection". The hardline scanner uses no `bashlex` — nothing in `agentao/` imports it — and does not follow indirection at all: measured against `hardline_check`, `rm -rf /` and `timeout 5 rm -rf /` are denied, while `D=/; rm -rf $D`, `D=rm; $D -rf /` and `X=-rf; rm $X /` all pass. So the gap in (1) is shared by the floor, not already solved there, and closing it for either is variable propagation that needs its own design first. (2) needs the runtime to know the user's home dir at policy-evaluation time, which adds host coupling we may not want; defer that piece until a concrete attack surfaces.
