...
最简正确路线：在 `session_set_mode` 里、`session.mode_id = mode_id` 之后，经会话 transport 发
`{sessionUpdate:"current_mode_update", currentModeId: mode_id}`。（transport *可以*也映射
`PERMISSION_MODE_CHANGED`，让未来 runtime 内部切换也能冒出来，但 handler 不得依赖它。）
...
```
**不要无条件 drop `TOOL_COMPLETE`。** 只在 **`status == "ok"`**（plan 已落地）时 drop `todo_write`
的 `TOOL_COMPLETE`。若调用失败，**保留** `status:"failed"` 的 `tool_call_update`，让 client 知道
plan **没**落地——否则它会一直显示 `TOOL_START` 那个"已生效"却从未真正应用的 plan。
...
- `agentao/acp/transport.py`：防御性 `todo_write`→`plan`（校验 entries；只在 `status=="ok"` 时 drop
  `TOOL_COMPLETE`）。可选地也映射 `PERMISSION_MODE_CHANGED`→`current_mode_update` 求完整——但它对
  ACP 路径不是承重的（见 §4.1）。
...
- 测试：`tests/test_acp_transport.py`（plan 正常路径 + 畸形 todos 回落 + 失败调用保留
  `tool_call_update`；模式变更发 `current_mode_update`）、session/new typed-modes 断言、schema 快照。
...
2. **`todo_write` 的 `TOOL_COMPLETE`** —— *已决。* 只在 **`status=="ok"`** 时 drop；失败时保留
   `tool_call_update:failed`，免得 client 看到一个"已生效"却未应用的 plan（§4.2）。plan 在
   `TOOL_START` 发（调用时列表即终态；`todo_write` 同步）。
