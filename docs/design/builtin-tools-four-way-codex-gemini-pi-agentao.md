...
**Status:** analysis, **rev 15** (2026-09-01).
...
gemini-cli**: `isGemini2Model` is `/^gemini-2(\.|$)/.test(model)` (`core/src/config/models.ts:458`), a regex
...
- **A citation's basename is not an address.** rev 1 wrote `apply_patch.rs:73`, `config.ts:1135` and
  ...
  files in pi-mono, and §2.4's contrast between its two packages is clearer with full paths. A bare basename that happens to be unique in its repo *resolves*, so every
