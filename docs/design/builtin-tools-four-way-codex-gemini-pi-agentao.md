...
**Status:** analysis, **rev 12** (2026-09-01).
...
| Hosted + extensions | `web_search` (provider-side), `web.run`, `image_gen.imagegen`, `skills.{list,read}`, `memories.{list,read,search,add_ad_hoc_note}`, `history.*`/`notes.*` (9), `get_goal`/`create_goal`/`update_goal` | `tools/hosted_spec.rs:14`; six `ToolContributor` impls under `ext/` |
...
`browser` (config-gated) — `agents/registry.ts:286-313`.
...
(`src/extensions/index.ts:4`). `todo` exists as `examples/extensions/todo.ts`, a sample.
...
(`core/src/tools/handlers/apply_patch.rs:73`), and `write_permissions_for_paths` (`:236-270`) derives an
...
(`handlers/view_image_spec.rs:19`), resolves it against the environment cwd, and reads it through the sandbox
...
call unchanged (`agent-loop.ts:617-624`). The permission gate ships as an **example**
(`examples/extensions/permission-gate.ts:13`), not as policy. The accurate row is: **no default
...
(`model_info.rs:142`), which logs *"Unknown model {slug} is used. This will use fallback model
...
gemini-cli**: `isGemini2Model` is `/^gemini-2(\.|$)/.test(model)` (`config/models.ts:458`), a regex
...
`options?.maxTokens ?? model.maxTokens` before clamping (`ai/src/api/simple-options.ts:34`), so an
...
|---|---|---|
| codex's per-turn model-capability gating | **tabled**, demand-gated | Real gap (§4). Closing it means agentao owning **some** continuously maintained compatibility fact — rev 5 said "a per-model table", which over-specified it: gemini-cli's is a regex (`config/models.ts:458`) and codex mixes catalog with `provider.capabilities()`. rev 5 also asserted agentao *cannot* maintain one provider-neutrally; nothing here supports that, and §4 records that the question has never been put — so this is tabled on **unmeasured demand**, not on established impossibility, and not on a fixed implementation shape. Revisit if a concrete model rejection is observed in the wild. |
