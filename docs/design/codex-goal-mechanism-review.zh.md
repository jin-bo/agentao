...
`/goal` 接口草拟的候选设计拆分(§11)。**§11 已获批准并随 0.4.13 发布**(2026-06-24,
即本文起草次日)—— 见 §11 的 restatus 横幅。§10 保持探索性、未经验证,也从未据此动工。
拆解部分独立成立;候选设计中,**§11 的具体论断在起草时已对 `agentao/` 做过 grep 核实**。
(§11 并未像草稿设想的那样拆成独立的实现计划文档 —— 它直接进入实施,所以本文同时也是
`/goal` 的设计记录。`agentao/cli/commands/goal.py:6` 反向引用 §11.1 为接口规格,这正是
本状态行不能再写「未实现」的原因。)
...
## 11. 三层拆分 —— **已获批准并随 0.4.13 发布**

> **Restatus(2026-09-19)。** 本节起草时是候选方案(「非已批准计划」)。它获得批准并
> **完整实施、随 0.4.13 于 2026-06-24 发布** —— 就在起草的次日 —— 而本节标题、下方两处
> `状态:` 块、以及 `docs/design/README.md`,历经 0.4.14 到 0.5.3 的**十八个版本**
> 一处都没改。三层全部落地:
>
> - **第 1 层** —— 以「无需改动」的方式按设计实现;`CHANGELOG.md` §0.4.13 明确写了
>   harness 保持 goal-agnostic。
> - **第 2 层** —— `agentao/cli/commands/goal.py`、`agentao/cli/goal_state.py`
>   (`.agentao/goal.json`)、`agentao/cli/input_loop.py::run_goal_continuation`,
>   以及注入的 `update_goal` 工具(`agentao/tools/goal.py`)。
> - **第 3 层** —— `developer-guide/{en,zh}/part-4/8-orchestration-continuation.md`,
>   路径与本节提议的完全一致,中英双份俱全。用户指南:`docs/guides/goal.md`。
>
> 下文原样保留为设计记录 —— 它是**已建成之物**,不是提案。文中「候选」「不是承诺」
> 等措辞按历史文本理解。

**状态:** ~~仅候选形态,记录供评估。~~ **已获批准并随 0.4.13 发布**(2026-06-24)。
起草时对 `main`(2026-06-23)做过 grep 验证。
...
**状态:** ~~供评估的草案;未批准/未实现。~~ **已随 0.4.13 发布**(2026-06-24),基本按草案
原样落地 —— 包括 C 段的默认值(`_DEFAULT_MAX_TURNS = 25`、`_DEFAULT_TIME_BUDGET = "120m"`,
见 `agentao/cli/commands/goal.py:35-36`)。**`agentao/cli/commands/goal.py:6` 反向引用本小节
为接口规格**,因此应按现行契约阅读,而非草案。范围 = **CLI(参考 host)** 的命令面 + 状态模型 +
