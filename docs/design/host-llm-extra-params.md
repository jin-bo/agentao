...
- **Runtime mutation (`/param`).** A *general* setter — `LLMClient.update_extra_body(**kw)` plus a CLI `/param set seed 42` / `/param show` — is out of v1. The use cases (`top_p`, `seed`, `response_format`) are static per session and fully served by the construction-time paths. **Partially landed since:** `/thinking [minimal|low|medium|high|off]` (`cli/commands/provider.py::handle_thinking_command`) is a *scoped* runtime setter for the one `extra_body` key users routinely retune mid-session — `reasoning_effort` ("thinking depth"). It mutates the live client's `extra_body` directly (propagating to sub-agents via the live `_llm_config` snapshot); `off` deletes the key. No general `/param` and no `update_extra_body` helper — `/thinking` writes the dict in place. The §"no auto-recovery" caveat applies: a stale `reasoning_effort` after a model switch still 400s until `/thinking off`.
