...
> 每一行都已落地，§4（ACP）也在内，取值以 `docs/reference/configuration.zh.md` §11 为准。
> 引用这张表时请连这句一起引。

**状态：** **已随 0.4.23 发布**（2026-09-11）——§6.1 / §6.2（CLI 取值 + 恢复路径）、
§4（ACP）、§6.3（压缩），三批各经一次维护者评审后分别放行，另加第四批：这三批暴露出来的用户提示路由。
本文从此是**已落地行为的依据**，不再是提案。
