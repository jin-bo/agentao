...
> **更新：** 第一项（§2，后台 ASK 固定拒绝）已于 2026-09-15 实施，随 0.4.24 发布；第二项（§3，并发上限）仍未授权。
> **更新（2026-09-26）：** §4.2 的观察已有真实案例并已处理——见 `background-subagent-wake.md`，#351–#353 合入 main（0.5.6 周期，未发布）。
...
| 若实施，优先级 | 内容 | 依据 |
|---|---|---|
| **P1（已实施，0.4.24）** | 后台子 agent 的 ASK **固定拒绝**（fail-closed）；前台不动 | §2 |
| **P1（未实施）** | `BackgroundTaskStore` 原子并发上限，满额直接拒绝 | §3 |
| **观察** | 上下文截断是否产生真实失败案例 | §4.1 |
| **观察 → 已处理（#351–#353）** | 轮询是否产生可量化的 token / 延迟代价——真实案例是父级用 `sleep` 等待、触发死循环检测后停住；按 `background-subagent-wake.md` 处理 | §4.2 |
| **不建** | 审批队列、三态审批配置、park / mailbox 子系统、`fork_turns`、residency 逐出、`AgentPath` 全局注册表、agent 间通信、dashboard | §5 |

---

## 2. 【P1，已实施 2026-09-15，随 0.4.24 发布】后台子 agent 的 ASK 固定拒绝

> **2026-09-15 随 #238/#239 的 PR 实施**，做法与落地时发现的前置缺陷（子代理的权限引擎从未生效）
> 见 `openworker-borrow-review.zh.md` §1 开头的注记。本节以下为原分析，保留不改。
...
> **已处理（2026-09-26）。** 观察到的案例比「轮询」更糟：父级用 `run_shell_command("sleep 300; …")` 等三个
> 后台子代理，第三次相同调用触发死循环检测后这一轮结束，子代理早已完成的结果要到用户手动输入
> `continue` 才被读到。处理方案见 `background-subagent-wake.md`，均已合入 main（未发布）：
> 启动文案改为「不要 sleep / 反复查询，没事可做就结束本轮」（#351）；交互式 CLI 空闲时自动开一轮读取
> 结果（宿主职责，与下文「等待策略更接近宿主职责」一致；#351，连续唤醒上限 3 次见 #352）；
> 以及下文预言的最小演进——`check_background_agent(wait_seconds=…)`，可取消、上限 1800 秒（#353）。
> 这个工具正是下文说的「第一个」轮询 `_cancellation_token` 的内置工具。参数名最终用 `wait_seconds`
> 而不是 `timeout_ms`，上限也不是「短时」：本例子代理还剩 7–17 分钟，短时等待覆盖不了。
> 以下为原文，保留作为当时的依据。
