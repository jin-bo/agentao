...
> **⚠️ 计划，已评审十二轮，未获授权实施。** 下面的 PR 顺序是**依赖顺序，不是排期**。全文没有一行已经动工。
> 引用 PR 表时请连同这一行一起引用。

**日期：** 2026-08-23
**状态：** 计划，rev 13——十二轮维护者评审已**折叠进正文**（§9 是记录，不是需要叠加阅读的勘误层；正文自身即为准）。
...
- **§3.1 会减弱**：若 `{"trigger": ...}` matcher 在所有已发布插件里都没人用，它仍是契约缺陷，但降为
  P2。降级前请重跑 marketplace manifest 的 grep。
...
十二轮评审的条目均已折叠进上文正文。此处仅作历史记录，**不是**覆盖层：§§1–8 自身即为准，任何一节都不需要
读本节才能读对。
