...
> **✅ 已实施 —— 2026-08-24。六个 PR 全部按下方依赖顺序合并进 `main`。** 尚未发布，目标 0.4.20。
>
> | | PR | 在 `main` 上的 squash 提交 |
> |---|---|---|
> | PR-1 | #187 | `8d38e0b` |
> | PR-3 | #188 | `05b6c93` |
> | PR-2 | #189 | `2661ff1` |
...
> | PR-6 | #192 | `d4c95ad` |
>
> 被它替换掉的那条横幅写的是「已评审十二轮，**未获授权实施**——全文没有一行已经动工」。此处记录而非删除，
> 因为另有七处文档引用它。
>
> **有两处有意偏离 PR 切分**，各自 PR 说明里都写明了：PR-3 一并落地了 `_run_compaction` 的 `cancel` /
> host 摘要分支（留下一个不兑现 `cancel` 的 `decide` 参数比 PR 大一点更糟，而且 §4.2 的状态表本来就钉死在
> PR-3 **之前**），以及 `COMPACTION_SETTLED` 的 replay 投影（不做的话 PR-3 只会**删掉**审计记录——溢出路径
> 那条无条件的 `CONTEXT_COMPRESSED`——而不给替代品；replay schema 1.2 → 1.3）。
>
> **设计看 §§1–8，实际落地看六个 PR 说明。** 两者不一致时，以 `main` 上的 PR 为准。

**日期：** 2026-08-23
**状态：** **已实施**（2026-08-24，尚未发布，目标 0.4.20）——rev 14，十二轮维护者评审已**折叠进正文**（§9 是记录，不是需要叠加阅读的勘误层；正文自身即为准）。
