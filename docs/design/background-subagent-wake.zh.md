...
**状态：** A、run 宿主改动与 C 已于 2026-09-26 **实施**（#351，连续唤醒上限见 #352）；B 已于 2026-09-26 **实施**，上限 1800 秒。均未发布，0.5.6 周期。B 的 ACP 门槛（§6.2）仍未关闭：目标客户端的轮次超时和进度显示尚未核对，所以上限仍是暂定值。
...
- **实现说明：** 同一并行批次里的两次等待会依次进行，而不是同时进行：执行器对每次调用持有
  按工具*实例*划分的锁（保护该实例的 `output_callback`），两次调用用的是同一个
  `check_background_agent` 实例，所以整个批次最多耗时各次等待之和。接受这一点：启动文案只要求
  等一次，同时等多个子代理不在本方案范围内（§7）。
