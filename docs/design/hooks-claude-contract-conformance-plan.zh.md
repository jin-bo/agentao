...
> **已随 0.4.21 发布**（2026-08-30），它就是那一版的头条。

**状态：** **已实施**（rev 24，2026-08-30）并**已随 0.4.21 发布**—— §8 的七个步骤与十个设计门全部完成。每个门结在哪里见 §0。
...
| **G1**（传输面） | 取计划里更省的那个选项 | **扩展 `PLUGIN_HOOK_FIRED` 载荷**，不新开事件类型：字段是 `user_notices`，要渲染 hook 通知的宿主从这里读。两个一方 surface **在生命周期事件上**不依赖它 —— 那几个直接路由（§5.2.1）—— 这正是省事那个选项站得住的原因。**事后更正：** 这句被读成了「没人需要消费者」，而该字段的其他生产者 —— `UserPromptSubmit`、`Stop`、`PostToolUse*`，以及后来的 `SessionStart(source="compact")` —— 分别在聊天循环、工具 worker 和压缩协调器里派发，三者都不拥有任何界面。它们的提示有这个载荷、没有读者，也就是同一个缺陷往下挪了一层。现在三个界面都会消费它（`tests/test_hook_notice_routes.py`） | §5.2.1、第 5 步 |
