...
1. **Visibility = "the next `chat()` / `arun()` call"**. `to_openai_format()` snapshots only *before* each `chat()` enters the inner LLM iteration loop (`_runner.py:201`), so a post-construction add/remove takes effect on the **next prompt/chat call**: *within the same `chat()`* (later LLM iterations, stop-hook re-entry) the **model-visible schema** does not change. This is a contract, not a defect — it makes "a consistent tool schema within a single call" an invariant (same source as plan-mode's `plan_*` filtering). Note the scope precisely: the freeze is on the *schema the model sees*. Tool **execution** resolves names against the live registry (`tool_planning.py:270` does `self._tools.get(name)` per call), so a mid-turn `remove_tool` *would* turn an already-emitted call into a lookup error. That window only opens if a host mutates mid-turn (from a concurrent task or inside a tool's `execute()`), which §7 puts out of scope for v1 — between-calls usage never hits it.
