...
- `docs/guides/embedding.md` — 嵌入模式实操
- `docs/reference/host-api.md` — `agentao.host` 公共 API 参考

---

## 1. Problem：为什么需要这份路线图

Agentao 在 0.3.1 落地了 `agentao.host` 公共契约后，下一步路线图经过多轮"和竞品比"的评审，**逐渐偏离了自身的嵌入定位**——AGENTS.md、`agentao serve` daemon、跨平台 sandbox、bench 平台等条目陆续被加入 P0/P1，但其中超过半数实际服务的是 CLI 用户、远程部署者或营销叙事，**而不是嵌入主体**（README 自我宣称的"local-first, private-first, embeddable AI agents"）。
...
| `agentao.host` 公共 API break 次数 | 0 | 0 | `tests/test_host_schema.py` |
...
| P0.4 | 公开 harness API typing gate：`agentao.host`、`Agentao.events()`、`active_permissions()`、能力注入参数对下游 strict type checker 友好 | additive | 3-5d |
...
- **Changes:**
  - 在当前 `## Quick Start`（`README.md:21`）之上插入新 `## Embed in 30 lines` 段，包含：
    ...
    2. 最小 pure-injection 片段（镜像 `docs/guides/embedding.md` 的「Pure-injection」块，**不要**用 env-discovery 的——pure injection 才是路径 A 的北极星）
    3. 一行链接到 `docs/guides/embedding.md` 与 `docs/reference/host-api.md`
...
- **Goal:** 下游用 `mypy --strict` 跑 `agentao.host` 零报错。harness 是兼容性边界，**必须**对 strict type checker 干净。
- **Files:**
  - 审计：`agentao/host/__init__.py`、`agentao/host/models.py`、`agentao/host/events.py`、`agentao/host/projection.py`、`agentao/host/schema.py`
...
- **Changes:**
  1. `mypy --strict --package agentao.host` 跑出来的每个错误都修（不能 `# type: ignore`）。
  2. 公开签名里所有 `Any` 替换为具体的 `Protocol`/Pydantic 类型。能力 protocol 已在 `agentao/capabilities/` 下；把公共部分从 `agentao.host` 重新导出，让宿主只走一条 import 路径。
  3. 新增 `agentao/host/protocols.py`，重新导出 `FileSystem`、`MCPRegistry`、`ShellExecutor`（当前在 `agentao.capabilities` 下），嵌入用户不必摸 `agentao.capabilities.*`。
  4. `docs/reference/host-api.md` 的 import 示例同步更新。
- **Accept:**
  - `uv run mypy --strict --package agentao.host` 退出码 0。
  - 下游示例仓（在 P0.6 中创建）开 `strict = true` 跑 wheel 通过。
- **Tests:**
  - `tests/test_host_schema.py` 增加运行时断言：`agentao.host.__all__` 与 `docs/reference/host-api.md` 列出的集合一致（drift 检测）。
  - 新 `tests/test_host_typing.py`：subprocess 跑 `mypy --strict` 一段 import 全部公开面的小脚本；dev 组没装 mypy 时跳过。
...
- **现有资产（不要重复造）：** `examples/host_events.py`、`examples/headless_worker.py`、`examples/batch-scheduler/`、`examples/data-workbench/`、`examples/ide-plugin-ts/`、`examples/saas-assistant/`、`examples/ticket-automation/`。可作积木，但都不是我们要的「最小宿主单文件」样本。
- **Files（新增）：**
...
- **现有测试（审计；不要重复造）：** `test_host_event_stream.py`、`test_active_permissions.py`、`test_host_permission_events.py`、`test_host_subagent_events.py`、`test_host_tool_events.py`、`test_host_schema.py`、`test_filesystem_capability_swap.py`、`test_mcp_registry_swap.py`、`test_shell_capability_swap.py`、`test_memory_store_swap.py`、`test_skill_manager_injection.py`、`test_mcp_manager_injection.py`、`test_llm_client_logger_injection.py`、`test_factory_build_from_environment.py`、`test_async_chat.py`、`test_no_subsystem_fallback_reads.py`、`test_per_session_cwd.py`。
- **新增测试：**
...
- **Files:**
  ...
  - `agentao/host/projection.py` — 加一个 sink，把每个 `HostEvent` 翻译成对应的 `ReplayEvent`，在 recorder 接好时投递
  - `agentao/replay/recorder.py` — 在允许集中接受新 kinds
- **Changes:**
  - schema 版本到 `1.2`。v1.0 / v1.1 schema 冻结，继续校验旧 replay——`docs/reference/replay-schema-policy.md` 的向后兼容承诺保持。
  - 三个新 kind 的 payload 形状借用 `agentao/host/models.py`（已是 Pydantic）——用 `model_json_schema()` 生成 JSON-Schema 片段，作为 `_kind_variant` 的每个 kind 变体内嵌进去。
- **Accept:**
...
- **Tests:** 扩展 `tests/test_replay_schema.py` 与 `tests/test_event_schema_version.py`；新增 `tests/test_host_to_replay_projection.py`。
...
| P0.4 typing gate | **已落地（PR 1，工作树）** | `mypy --strict --package agentao.host` 干净；新增 `agentao/host/protocols.py` re-export；CI `Typing gate` Job 强制；`tests/test_host_typing.py` 覆盖包 + 模拟下游消费者 + `__all__` drift |
...
| # | 分支 | 范围 | 依赖 | 净新增 diff（估） |
|---|---|---|---|---:|
| **1** | `roadmap/p0-4-typing-gate` | 仅 P0.4 —— `mypy --strict --package agentao.host` 干净；新增 `agentao/host/protocols.py` re-export；CI step | — | ~250 行 |
...
| **4** | `roadmap/p0-8-replay-v1-2` | 仅 P0.8 —— replay schema v1.2、harness→replay projection、`tests/test_host_to_replay_projection.py` | —（独立） | ~300 行 |
...
| PR | 本 PR 引入的新关卡（必须绿） |
|---|---|
| 1 | `uv run mypy --strict --package agentao.host` 退出 0；`tests/test_host_typing.py` 在 dev 内可跑 |
...
| 4 | `scripts/write_replay_schema.py --check` 对 v1.2 干净；`tests/test_host_to_replay_projection.py` 绿 |
...
>
> **Cost:** 对现有用户零影响——`agentao` 在 `[ai]` extra、由环境变量控制、可完全卸载。[链接到对应 Path A 保证：`docs/guides/embedding.md` 讲能力注入、`docs/reference/host-api.md` 讲公开 API。]
>
