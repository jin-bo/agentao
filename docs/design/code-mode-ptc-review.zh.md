...
路线差异与启动条件。opencode 这个数据点**强化而非推翻**当前结论（§6、附录 F 末）。
...
连成熟厂商 CLI 都把它当可选 / 实验 / 或不做（goose 与 opencode 均默认关闭，gemini-cli
不做）—— 佐证 Code Mode 是"按需上"而非标配（gap≠need）。
...
| 能力边界 | 无 Node / 文件 / 网络 / console —— 语言层就够不到外部 | 本地后端文件 / 网络开放（靠环境擦洗 + 输出脱敏兜底）；远程后端由容器隔离 | 仅注册当前已启用扩展的工具；可通过回调触达文件、网络或 shell 类工具 | 语言层就够不到 fs/进程/网络/module —— 全局作用域是**手写白名单**（只有 `tools` + 纯数据 stdlib） |
| 隔离 | 默认独立进程（hosted），失败回退进程内 V8 | 进程级（本地）/ 容器（远程） | 没有独立进程或容器边界；裸 TypeScript 是否具有直接文件 / 网络能力，不能仅从 Goose 集成层断言 | **无进程/容器边界，也不嵌引擎**——隔离＝解释器本身（能力靠"构造上不存在"）；**天然跨平台** |
...
| 默认状态 | 内建 | 内建（cron / 无人会话默认禁用） | **默认关闭** | **默认关闭**（`experimentalCodeMode` flag 后；首版曾 revert 重做） |

> 注：goose 的 `execute_bash` 应称为 pctx 提供的**元工具**，不能直接等同于 Goose 的宿主
> shell 工具。

四条路线可归纳为：**codex 与 opencode 都"约束解释器"**（语言层够不到外部），但**codex 嵌真
V8 再削全局**（default-allow-then-strip，漏删一个＝逃逸，安全押在 V8 健全性上）、**opencode
自建解释器只注入白名单全局**（default-deny-by-construction，无底层引擎可逃、无需 OS 沙箱、
天然跨平台，代价是须手写维护解释器 + stdlib 且只支持 JS 子集）；**hermes 约束进程 + 事后
兜底**（真子进程 + 环境擦洗 + 输出脱敏 + 整脚本审批）；**goose 约束时间维**（超时 + 取消，
但无空间隔离）。它们对"信任模型"的选择不同，没有唯一正解。
...
2. **能否建立可信的执行边界。**（专用沙箱 + 整脚本审批 + 无人值守默认拒绝；跨平台隔离
   ...
   opencode 证明了一条**不依赖 OS 沙箱、天然跨平台**的边界确实存在——自建受限 JS 子集
   解释器；但这条路对 Python 宿主**不便宜**（自写受限 Python 子集解释器远难于 JS，
   `RestrictedPython` 素以易漏著称），故它只是把"跨平台"从抱怨变成一个**已验证可行、但
   对 Python 代价过高**的明确取舍项，不改变本文对 Python 宿主"OS 沙箱更务实"的判断。
   详见附录 F。）
...
（**opencode 提供了第三种主边界**：既不嵌 V8、也不开子进程，而是自建受限解释器 —— 见附录 F。
它在"无引擎逃逸面 + 跨平台"上比 codex 更进一步，但对 Python 宿主代价过高。）
...
- **codex ＝ 嵌真 V8 再削全局**（`install_globals` 删 `console`/`Atomics`/`WebAssembly`…）——
  default-allow-then-strip，漏删一个＝逃逸；安全押在 V8 健全性上（V8 逃逸是历史攻击面，靠
  hosted 进程回退兜底）。
- **opencode ＝ 自建解释器只注入白名单** —— default-deny-by-construction，忘加一个＝少个功能
  （不是漏洞）；**没有底层引擎可逃**，也**无需 OS 进程/沙箱**，因而**天然跨平台**。代价：
  (1) 须手写并维护解释器 + stdlib 的正确性（一处把宿主对象漏进作用域即逃逸）；(2) 只支持它
  实现的 **JS 子集**，非完整 JS。
...
**安全姿态小结：** opencode ≈ **codex 的"脚本除调用受审批工具外什么都做不了"结构上限，但用
自建解释器替掉 V8、且不需要任何 OS 进程隔离**。在"无引擎逃逸面 + 跨平台"这一维上它比 codex
更进一步；代价是自写解释器的正确性负担与语言子集限制。若把它放进附录 E.1 那张表，opencode
与 codex 在"能否绕过单工具审批＝不能""整脚本审批门＝不需要""密钥暴露面＝近乎零"三行完全同列，
仅"约束脚本的是什么"从"V8 isolate 本身"换成"自建解释器本身"、"进程隔离深度"从"hosted 进程"
换成"无（不需要）"。

**对 agentao 的含义（回流 §5 开放问题 #2 跨平台成本）：** opencode 证明了一条**不依赖 OS
沙箱、天然跨平台**的可信执行边界确实**存在** —— 这正击中 agentao 现有推荐（hermes 式子进程
+ `sandbox-exec`）的软肋：`sandbox-exec` 仅 macOS。**但这条路对 agentao 不便宜**：opencode
能自建，是因为写一个受限 **JS 子集**解释器可行；agentao 是 Python 宿主，等价物是自写受限
**Python 子集** AST 解释器 —— Python 语义远重（描述符 / dunder / 元类 / 生成器 / 异常），
`RestrictedPython` 素以易漏著称，维护负担也更大。故 opencode 这个数据点**强化而非推翻** §6
结论：对 Python 宿主，OS 沙箱路线仍是更务实的一步；opencode 的价值在于把"跨平台"从一句抱怨
变成一个**已被验证可行、但对 Python 代价过高**的明确取舍项。
