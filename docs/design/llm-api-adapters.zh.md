...
**阶段 2 的 `openai-responses` 已于 2026-09-19 实施（已随 0.5.3 发布；见下文「阶段 2 落了什么」）—— 验证对象是
...
**在 Anthropic 自己的 API 上，缓存收益只有走原生线路才*量得出*。** 它在兼容端点上是否也*拿得到*
—— 靠标记，或靠端点不经请求自己做的缓存 —— 两个方向都没有证据：一个什么都不上报的端点，并没有
被证明什么都没缓存。（这句话的初版写的是「才拿得到、也才量得出」；证据只支持后半句。）0b 的价值在于那些
...
**第二次实测，用这个脚本跑（同一天、同一端点，`claude-sonnet-5`）。**

| 组 | 线路 | 请求数 | 输入 token | 缓存写入 | 缓存读取 | 输入成本单位 |
|---|---|---|---|---|---|---|
| A —— 仅 0a | Chat Completions | **19** | 292,524 | 未上报 | 未上报 | ≤ 292,524 |
| B —— 0a + 0b | Chat Completions | 11 | 197,511 | 未上报 | 未上报 | ≤ 197,511 |
| C —— 原生 | `anthropic-messages` | 11 | 198,881 | 21,879 | 176,980 | **45,069**（对自身全价 −77%） |

它复现了第一次的结论 —— −77% 对 −78% —— 并且更精确：原生线路上**整段会话按全价计费的只有
22 个 token**，每个请求 2 个；第一个请求之后，每个请求都把上一次的整段前缀读了回来。B 与 C 走的
路径完全一样（11 个请求，每一轮的请求数都相同），所以这一对是同口径的：≤ 197,511 对 45,069。
**A 组的总量不可比**：那一次模型把读文件拆成了更多轮工具调用，发了 19 个请求。这是第一次实测
没有暴露出来的方法缺陷 —— 一轮要走几个来回由模型决定，而它两次的决定并不相同 —— 所以脚本现在
报的是每一组对**它自己**全价的降幅，并在各组请求数不一致时指出来，不再打印跨组差值。这次的
提示收尾时比第一次长（9.6k → 21.9k，对 9.6k → 17.9k），原因没有查；逐请求的模式是一样的。

还剩三个问题，重跑这张表一个也回答不了：

1. **Anthropic 兼容端点上 A 与 B 之分**只在账单里，别处看不到。只有账号持有人能查。
2. **网关是否认标记。** 对一个网关有了答案，其余仍未知。一个后端为 `deepseek-v4.1-flash` 的
   Anthropic 兼容端点，原生线路打断点（C）对同一条线路不打断点（D）：

   | 组 | 请求数 | 输入 token | 缓存写入 | 缓存读取 | 命中率 |
   |---|---|---|---|---|---|
   | C —— 原生，3 个断点 | 14 | 204,196 | 0 | 181,504 | 88.9% |
   | D —— 原生，**不打**断点 | 19 | 271,411 | 0 | 248,320 | 91.5% |

   **在这个端点上，标记没有带来任何可测量的差别。** 不打标记的那一组命中率并不更低，而且两组是
   同一种特征 —— 每次读取量都是 64 的整数倍、从不上报写入 —— 即按块的自动前缀缓存。不管它有
   没有读到任何一个 `cache_control`，上报的读取量都一样；这也是先前只跑 C 组的那一次（17 个
   请求，91.5%）定不了这件事、脚本因此加了 D 组的原因。在这类端点上起作用的是 **0a** 阶段：
   被缓存的就是那段逐字节稳定的前缀，对端点没有任何要求。`prompt_cache` 在这里买不到可证明的
   东西，保持关闭。两点提醒：可比的是命中率而不是总量（模型在一组里走了 14 个来回、另一组
   19 个）；默认的 `--read-rate 0.1` 是 Anthropic 的价 —— 要换成网关自己的命中价。

   **同一个问题放到 Chat Completions 上** —— 同一厂商的 OpenAI 兼容端点、同一个模型，不打
   断点（A）对三个 `cache_control` 标记（B）：

   | 组 | 请求数 | 输入 token | 缓存写入 | 缓存读取 | 命中率 |
   |---|---|---|---|---|---|
   | A —— 仅 0a | 15 | 233,169 | 0 | 211,712 | 90.8% |
   | B —— 0a + 0b | 14 | 206,074 | 0 | 185,088 | 89.8% |

   答案相同，特征也相同。标记被接受、没有报错，也没有带来任何可测量的变化。这个端点*会*上报
   —— 走的是 `prompt_tokens_details.cached_tokens`，这是 0.5.1 新增的那条读取路径第一次在
   真实端点上读到数 —— 所以和 Anthropic 的兼容端点不同，这里 A、B 分得开，而它们并没有差别。
   四组合起来，无论哪条线路、打没打标记，命中率都落在三个点之内（88.9–91.5%）。**这是同一厂商
   的两个端点、同一个模型**，不是关于「网关」的结论：一个只实现 Anthropic 式断点缓存、没有自动
   缓存的网关，会表现为 C 高于 D，而这样的网关还没有测过。
