...
2. **网关是否认标记。** 目前只跑过一个网关，而且它回答不了这个问题：一个后端为
   `deepseek-v4.1-flash` 的 Anthropic 兼容端点，C 组，17 个请求 —— 输入 250,766 token，
   **写入 0、读取 229,504（91.5%）**。读取量全是 64 的整数倍（6,400、9,216、12,288……），
   而且从不上报写入 —— 这是按块的自动前缀缓存，不管它有没有读到任何一个 `cache_control`
   标记，上报的读取量都会一样。所以在这类端点上，单看 C 组对标记一无所说；脚本为此加了 **D 组**
   （同一条线路、不打断点）作对照，**尚未跑**。这次运行能说明的是：在这个网关上，0a 做成逐字节
   稳定的那段前缀确实被缓存了，但看不出标记起了作用；另外默认的 `--read-rate 0.1` 是 Anthropic
   的价 —— 换成网关自己的命中价才对。Chat Completions 类网关的那一对仍是
   `--arms a,b --base-url-compat <网关>/v1`，尚未跑。
