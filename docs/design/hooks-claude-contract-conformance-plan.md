...
| **G10** | plan's proposal, taken at implementation | **Session-scoped, lock-guarded, keyed by a content-derived rule key** — never `id(rule)`, which changes on reload and would silently re-announce everything. Dispatcher scope was the trap: the dispatcher is built fresh per dispatch — **nine** construction sites across six files, two of them inside pool workers (earlier revisions said "six sites", counting files) — so its state would dedup nothing and race while doing it. `clear_session()` on a plugin reload and on `/clear`, so a corrected hook speaks up again while an unchanged one stays quiet | §4.2, step 2 |
...
| **G1** (transport) | plan's cheaper option | **An extended `PLUGIN_HOOK_FIRED` payload**, not a new event type: the field is `user_notices`, and a host that renders hook notices reads it there. The two first-party surfaces do **not** depend on it *for the lifecycle events* — those route directly (§5.2.1) — which is what made the cheap option viable. **Corrected after the fact:** that read as "nothing needs a consumer", and the field's other producers — `UserPromptSubmit`, `Stop`, `PostToolUse*`, and later `SessionStart(source="compact")` — are dispatched inside a chat loop, a tool worker and the compaction coordinator, none of which owns a surface. Their notices had this payload and no reader, i.e. the same defect one layer along. All three surfaces now consume it (`tests/test_hook_notice_routes.py`) | §5.2.1, step 5 |
...
- **G10 — the diagnostic registry (§4.2).** Blocks step 2, where `diagnostics[]` first has a producer.
  Owner (session-scoped, not dispatcher-scoped — it is built fresh per dispatch at nine sites
  across six files, two of them inside pool workers), the lock, the **stable rule key** that survives a plugin reload, and the lifecycle on
