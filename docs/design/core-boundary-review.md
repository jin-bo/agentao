...
> **Status, 2026-05-07:** items #1, #2-via-#1-side-effects (no separate work — see below), #3 (session.py migration), and #4 still call permission engine API redesign — landed in commits `838a952` (#3) and `0310eda` (#1). Original priority table preserved below; checkboxes added.
...
2. **`Agentao.__init__` callback signature.** Move all 8 deprecated callbacks (the 7 commonly known plus `on_max_iterations_callback`) out of the public constructor, route through `embedding/compat.py`. `build_compat_transport` already lives in `transport/sdk.py:82`; this is API tightening, not physical relocation. ~1 day.
...
4. **Permissions file I/O up to `embedding/`.** This is an **engine API redesign**, not a half-day file move. `PermissionEngine.__init__` (`permissions.py:297-346`) currently calls `self._load_rules()` itself, which reads `<user_root>/permissions.json` (`permissions.py:368-388`). Four caller sites currently pass `project_root` + `user_root` and rely on the engine to load: `embedding/factory.py:141`, `agents/tools.py:585`, `acp/session_new.py:306`, `acp/session_load.py:199`. Either (a) change the constructor to accept pre-loaded rules and update all four callers, or (b) extract `_load_rules` to a classmethod / factory function and have callers invoke it explicitly. ~1–1.5 days, Medium risk (touches embedding + ACP + agents).
