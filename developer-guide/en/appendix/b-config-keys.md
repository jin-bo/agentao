...
| `{PROVIDER}_API_FORMAT` | — | `openai-completions` | The wire protocol spoken to that block's endpoint (0.5.0): `openai-completions`, `anthropic-messages` (Anthropic's Messages API) or `openai-responses` (OpenAI's Responses API, 0.5.3). **Never inferred** from the URL, the provider name or the model name; an unknown value raises at startup. `/provider` reads the target block's value, so a switch between blocks on different protocols changes the wire too; sub-agents are built on the parent's current one. On `anthropic-messages`, `LLM_TEMPERATURE` is not sent and extended thinking is turned on through `LLM_EXTRA_BODY`. Equivalent to the constructor `api_format=`. Full notes: `docs/reference/configuration.md` §2 |
...
| `LLM_PROMPT_CACHE_TTL` | `5m` | Retention hint for those markers: `5m` or `1h`. Unread — validation included — while `LLM_PROMPT_CACHE` is off. Equivalent to the constructor `prompt_cache_ttl=` |
