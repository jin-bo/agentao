...
- `llm_client=` together with any of `api_key=` / `base_url=` / `model=` / `temperature=` / `max_tokens=` / `extra_body=` / `prompt_cache=` / `prompt_cache_ttl=` / `api_format=` (pass them to the injected client instead)
