...
| `api_format` | `str` | `"openai-completions"` | The wire protocol spoken to `base_url` (0.5.0): `"openai-completions"` or `"anthropic-messages"` (Anthropic's Messages API over the official SDK). **Keyword-only**, configured and never inferred from the URL or the model name, fixed for the life of the agent, inherited by sub-agents, and **mutually exclusive with `llm_client=`**. An unknown value raises `ValueError`. On `anthropic-messages`, `base_url` is the API root, `temperature` is not sent, and extended thinking goes through `extra_body`. Env equivalent: `{PROVIDER}_API_FORMAT` — see [Appendix B](/en/appendix/b-config-keys) |
