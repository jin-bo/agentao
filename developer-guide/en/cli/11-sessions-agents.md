...
When a background task finishes while the REPL is waiting at an empty prompt, the CLI prints `⟳ background agent finished — continuing` and runs one turn so the main agent reads the result — whether the model launched the task or you did with `/agent bg`. It waits while you are typing, have images staged, or are in plan mode, and after 3 wakes in a row it pauses until you enter something. Turn it off with `"background_agents": {"auto_wake": false}` in `.agentao/settings.json`; the result then reaches the agent with your next message.
