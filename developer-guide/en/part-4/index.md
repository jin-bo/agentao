...
- **HarnessEvent** — Pydantic-typed lifecycle event (tool / permission / subagent); **stable**, with schema snapshots · [§4.7](/en/part-4/7-harness-contract), [G.1](/en/appendix/g-glossary#g-1-core-concepts)
- **`agent.events()`** — async pull iterator on the *stable* `agentao.harness` surface; use for audit / SIEM / billing · [§4.7](/en/part-4/7-harness-contract)
- **`active_permissions()`** — JSON-safe snapshot of the effective policy; use for "who can do what" UIs · [§4.7](/en/part-4/7-harness-contract#the-active-permissions-snapshot), [G.5](/en/appendix/g-glossary#g-5-security-vocabulary)
:::
...
- [**4.7 Embedded Harness Contract**](./7-harness-contract) — `agent.events()` + `active_permissions()` — the **stable host API** for production audit / observability pipelines
