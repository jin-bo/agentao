...
| `PERMISSION_MODE_CHANGED` / `READONLY_MODE_CHANGED` | Runtime safety mode changed. The first carries `previous` / `current` (mode strings) and `cause` — `"cli"`, `"cli-allow-all"`, `"cli-plan-implement"`, `"acp"`, `"run"` or `"host"`, naming the entry path that made the switch — and fires only when the preset actually moved. The second carries `previous` / `current` as **booleans**: it reports `ToolRunner.readonly_mode`, one of read-only's two switches, not the effective posture. Both are emitted by `agent.set_permission_mode(...)`; a bare `permission_engine.set_mode(...)` emits neither. `READONLY_MODE_CHANGED` also fires on its own from a direct `ToolRunner.set_readonly_mode(...)` — which is how a sub-agent inherits the flag. |
| `PLUGIN_HOOK_FIRED` | A plugin hook ran. `data["hook_name"]` is one of `UserPromptSubmit` / `PreToolUse` / `PostToolUse*` / `Stop` / `PreCompact` / `SessionStart`. Two of those read oddly and both are deliberate: `PostToolUse*` is **one name for two events**, because a batch mixes `PostToolUse` and `PostToolUseFailure` results and the notices are aggregated across it, so branching on either name alone never matches; and `SessionStart` appears here **only** after a full compaction (`source="compact"`) — a `SessionStart` / `SessionEnd` at a real session boundary emits nothing on this event and returns its result to the surface that dispatched it instead. The hook-name-specific fields differ — for example `Stop` carries `turn_end_reason ∈ {"final_response", "max_iterations", "doom_loop", "hook_stop"}`, `at_max_iter`, `added_context_count`, and `suppress_output`; `PreCompact` carries `compaction_type ∈ {"microcompact", "full", "minimal_history"}` and `trigger="auto"`. **`user_notices` is the field a host renders**: a list of strings a hook addressed to the *user* rather than the model — `systemMessage`, exit-2 stderr, the one-time profile field diagnostics — and for every event listed here it is the only route they have. Every emit also carries `outcome` and a rule count, in **two different fields**: `matched_rule_count` (`PreToolUse` / `Stop` / `PreCompact` — rules selected for dispatch, and those three emit nothing when it is zero) or `rule_count` (`UserPromptSubmit` / `PostToolUse*` / the compaction `SessionStart` — rules *configured*, since none of the three has a matched count in hand where it emits). For `Stop`, `outcome ∈ {"allow", "block", "continue", "continue_at_max_iter", "reentry_capped"}` reflects the chat-loop verdict at that exit site (`continue` vs `continue_at_max_iter` disambiguates which site honored a `force_continue` decision; `reentry_capped` means the loop refused a further re-entry). For `PreCompact`, `outcome` is always `"allow"` (observe-only). For the rule-author guide, see [§5.7 Plugin Hooks](/en/part-5/7-plugin-hooks). |
