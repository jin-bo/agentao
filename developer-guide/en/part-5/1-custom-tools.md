...
- misspelled tool names are repaired **only** when normalising case, separators, camelCase or a
  trailing `Tool` suffix reproduces a registered name exactly — a name that merely *resembles*
  one is reported as not found, and the model re-issues the call
