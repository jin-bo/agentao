...
`usage` 是本次运行各个 LLM 请求上报用量的总和：agent 自己的调用、它的压缩摘要请求，以及——自 0.5.1 起——它委派出去的每个**前台子代理**（此前子代理的请求不计入任何地方，所以有委派的运行会少报）。**`usage` 是一份快照，在写出 envelope 的那一刻读取**：`agentao run` 不等待后台子代理，所以及时结束的会被计入（自 0.5.2 起，它的用量在宣布完成之前就已加进累计），仍在运行的则不会——envelope 写出之后也不会再修订。需要按子代理拆分的数字时，宿主应改读 `SubagentLifecycleEvent.usage`。`prompt_tokens` 含缓存命中的输入，而 provider 对这部分按另一个价格计费；`cache_read_tokens` 与 `cache_creation_tokens`（0.5.1）说明其中有多少 —— 它们是 `prompt_tokens` **之中**的一部分，不是额外相加的量，也不会在 `total_tokens` 里再算一次。Agentao 只报这四个量，不套任何价格。流中失败的请求按服务端当时**已上报**的量计入（0.5.2）：在 `anthropic-messages` 线路上那是完整的输入量，流的第一个事件里就给了；Chat Completions 的 usage 只在最后一个 chunk 里，流在那之前断掉就什么也加不上。一次失败的尝试和随后的重试是两个请求，都会计入。上报不等于计费 —— 请把 `usage` 当用量，不要当账单。
