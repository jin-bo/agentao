...
- `from agentao.harness import ...` → `ActivePermissions`、`EventStream`、`StreamSubscribeError`、`HarnessEvent`、`ToolLifecycleEvent`、`SubagentLifecycleEvent`、`PermissionDecisionEvent`、`RFC3339UTCString`、`export_harness_event_json_schema`、`export_harness_acp_json_schema` —— 宿主面 harness 合约，详见 [A.10](#a-10-嵌入-harness-合约)
...
| `events` | `events(session_id: str | None = None) -> AsyncIterator[HarnessEvent]` | 订阅公共 harness 事件（工具/子 Agent/权限决定生命周期）。无 replay；有界背压。详见 [A.10](#a-10-嵌入-harness-合约) |
| `active_permissions` | `active_permissions() -> ActivePermissions` | 当前权限策略快照（`mode`、`rules`、`loaded_sources`），JSON-safe。详见 [A.10](#a-10-嵌入-harness-合约) |
...
`agentao.harness` 是嵌入 Agentao 的 **稳定宿主面 API**。内部运行时类型（`AgentEvent`、`ToolExecutionResult`、`PermissionEngine`）刻意 **不在** 该面内 —— 任何版本都可能改动。只针对 `agentao.harness`（加上 `Agentao(...)` 构造器以及上文标注的方法）开发的宿主可在版本升级中保持前向兼容。

完整参考：[`docs/api/harness.md`](../../../docs/api/harness.md) · [`docs/api/harness.zh.md`](../../../docs/api/harness.zh.md)。设计动机：[`docs/design/embedded-harness-contract.md`](../../../docs/design/embedded-harness-contract.md)。
...
| `HarnessEvent` | 三种事件模型的判别联合（Pydantic discriminator: `event_type`） |
...
| `export_harness_event_json_schema()` | 导出事件 + 权限面的标准 JSON schema。`tests/test_harness_schema.py` 用它与 `docs/schema/harness.events.v1.json` 做字节相等校验 |
| `export_harness_acp_json_schema()` | 导出宿主面 ACP 载荷的标准 JSON schema。快照在 `docs/schema/harness.acp.v1.json` |

### `agent.events(session_id=None)`

返回 `HarnessEvent` 的异步迭代器。传 `session_id=` 过滤；传 `None` 订阅该 `Agentao` 实例下所有会话。
...
- [`docs/schema/harness.events.v1.json`](../../../docs/schema/harness.events.v1.json) —— 事件 + 权限面
- [`docs/schema/harness.acp.v1.json`](../../../docs/schema/harness.acp.v1.json) —— 宿主面 ACP 载荷

`tests/test_harness_schema.py` 会从 Pydantic 模型重新生成 schema，并与快照做字节相等比对。任何改变 wire form 的 model 变更必须在同一 PR 内同时更新模型与快照。新增 optional 字段向后兼容；删除/重命名需要 schema 版本号 bump。
...
CLI 可基于同一组事件构建自己的 UI，但其本地存储与命令不会被提升为 harness API。
