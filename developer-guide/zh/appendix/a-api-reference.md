...
- `llm_client=` 与任何 `api_key=` / `base_url=` / `model=` / `temperature=` / `max_tokens=` / `extra_body=` / `prompt_cache=` / `prompt_cache_ttl=` / `api_format=` 同时传（改为把它们传给注入的 client）
...
`chat()` / `arun()` 接受 `images=[{"data": <base64>, "mimeType": "image/*"}, …]`
（0.4.8+）。每张图片以 OpenAI `image_url` data-URL part 随文本一起送入模型。
模型拒绝图片输入时，本轮自动重试一次，把图片退化为文本——每张图一个自闭合标签，
附在 user message 末尾：
...
`uri` 取已知的来源路径或 URL（例如经 CLI `/image` 命令暂存的文件）；wire 上不带
...
| `origin(name: str) -> str` | 读回记录的来源。子代理据此判定：`host` 工具只有在自身声明了 `copies_to_subagents` 时才会进入子代理，且以副本形式——这也包括用内置工具自己的类构造、替换了内置工具的宿主工具，正因如此判定不能看类身份。不经 `register` 放进 `tools` 的工具读作 `host`。名字不存在时抛 `KeyError` |
