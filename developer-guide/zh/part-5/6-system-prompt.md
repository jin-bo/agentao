...
- 系统提示**每轮重建**——不要假设它是个静态字符串可以缓存。
- 你拥有 11 个块中的 3 个：**`AGENTAO.md`**（项目硬规则）、**技能正文**（激活后的知识）、**自定义 Tool 的 description**（什么时候/怎么调）。
- 另外 8 个（日期、工作目录、可用工具/技能目录、记忆召回、todos 等）是运行时注入的，不该被覆盖。
