...
这一部分分成两条轴：**能力平面**让 Agent 获得业务能力、知识和规则；**控制平面**让你在 Agent 走到关键生命周期点时介入。5.1–5.6 与 5.8 是能力平面，5.7 是控制平面。
...
- [**5.8 宿主工具注入**](./8-tool-injection) — 从宿主侧选择工具面：`extra_tools` / `disable_tools` / `enabled_tools` + 运行期 `add_tool` / `remove_tool`
...
| 从宿主侧选择 / 收缩工具面 | [5.1](./1-custom-tools) → [5.8](./8-tool-injection) | 在构造期或运行期注入、替换、裁剪工具——并清楚它不是安全边界 |
