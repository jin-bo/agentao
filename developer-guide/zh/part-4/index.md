...
- **HarnessEvent** — Pydantic 类型的生命周期事件（tool / permission / subagent）；**稳定**，附 schema 快照 · [§4.7](/zh/part-4/7-harness-contract)、[G.1](/zh/appendix/g-glossary#g-1-核心概念)
- **`agent.events()`** — 落在**稳定** `agentao.harness` 表面上的异步 pull 迭代器；审计 / SIEM / 计费用 · [§4.7](/zh/part-4/7-harness-contract)
- **`active_permissions()`** — 当前生效策略的 JSON 化快照，给"谁能做什么"UI 用 · [§4.7](/zh/part-4/7-harness-contract#active-permissions-策略快照)、[G.5](/zh/appendix/g-glossary#g-5-安全术语)
:::
...
- [**4.7 嵌入式 Harness 合约**](./7-harness-contract) — `agent.events()` + `active_permissions()` —— 给生产审计 / 可观测流水线用的**稳定宿主 API**
