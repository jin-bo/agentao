...
| 字段 | 说明 |
|------|------|
| 触发时机 | turn 内**每次 LLM 迭代**前（同一个 turn 可能发很多次）；用户拒绝确认后也会发一次，用来重置显示 |
| `data` | `{}` 空对象；拒绝后的那次重置是 `{"display_reset": true}`，它不开始新的 LLM 迭代，不要把它算作一轮 |
...
| `PERMISSION_MODE_CHANGED` / `READONLY_MODE_CHANGED` | 运行时安全模式变化。前者带 `previous` / `current`（模式字符串）和 `cause`——`"cli"`、`"cli-allow-all"`、`"cli-plan-implement"`、`"acp"`、`"run"` 或 `"host"`，点名是哪条入口切的——且只在预设真的变了时才发。后者的 `previous` / `current` 是**布尔**：它报的是 `ToolRunner.readonly_mode` 这一个开关（只读有两个），不是生效姿态。两者都由 `agent.set_permission_mode(...)` 发出；裸调 `permission_engine.set_mode(...)` 两个都不发。`READONLY_MODE_CHANGED` 也会单独由直接调用的 `ToolRunner.set_readonly_mode(...)` 触发——子代理就是这样继承这个开关的。 |
| `PLUGIN_HOOK_FIRED` | 插件 hook 已运行。`data["hook_name"]` 取值为 `UserPromptSubmit` / `PreToolUse` / `PostToolUse*` / `Stop` / `PreCompact` / `SessionStart` 之一。其中两个看着别扭，但都是有意的：`PostToolUse*` 是**两个事件共用的一个名字**，因为一批调用里会同时出现 `PostToolUse` 和 `PostToolUseFailure` 的结果，提示跨整批聚合，按单名分支永远匹配不上；而 `SessionStart` **只**在 full 压缩之后出现（`source="compact"`）—— 真正会话边界上的 `SessionStart` / `SessionEnd` 在本事件上什么都不发，而是把结果返回给派发它的界面。各 hook_name 携带不同的字段——例如 `Stop` 带 `turn_end_reason ∈ {"final_response", "max_iterations", "doom_loop", "hook_stop"}`、`at_max_iter`、`added_context_count` 与 `suppress_output`；`PreCompact` 带 `compaction_type ∈ {"microcompact", "full", "minimal_history"}` 与 `trigger="auto"`。**`user_notices` 是宿主要渲染的那个字段**：一串 hook 讲给**用户**而非模型听的字符串 —— `systemMessage`、exit 2 的 stderr、一次性的 profile 字段诊断 —— 对这里列出的每一个事件来说，它都是唯一去路。所有 emit 都带 `outcome` 和一个规则数，但规则数是**两个不同的字段**：`matched_rule_count`（`PreToolUse` / `Stop` / `PreCompact` —— 被选派的规则数，这三个在它为 0 时**完全不发出事件**）或 `rule_count`（`UserPromptSubmit` / `PostToolUse*` / 压缩后的 `SessionStart` —— **已配置**的规则数，因为这三个在发出的位置上手里并没有匹配数）。`Stop` 的 `outcome ∈ {"allow", "block", "continue", "continue_at_max_iter", "reentry_capped"}`，反映该退出点的 chat-loop 判决（`continue` 与 `continue_at_max_iter` 用来区分是哪一个退出点接受了 `force_continue`；`reentry_capped` 表示循环拒绝再次重入）。`PreCompact` 的 `outcome` 仍恒为 `"allow"`（仅观察）。规则作者视角的完整指南见 [§5.7 插件 Hooks](/zh/part-5/7-plugin-hooks)。 |
