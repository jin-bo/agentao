...
| 你现在要做什么 | 最短路径 |
|---------------|----------|
| 先把 Python SDK 嵌进一个脚本 | [2.1 安装与包导入](./1-install-import) → [2.2 构造器最少参数](./2-constructor-reference) → [2.3 标准生命周期](./3-lifecycle) |
| 接进 FastAPI / Flask 产品 | [2.7 生产级模板](./7-fastapi-flask-embed) → [2.6 取消与超时](./6-cancellation-timeouts) → [2.4 会话状态](./4-session-state) |
| 做多租户 / 多会话池 | [2.2 构造器参数](./2-constructor-reference) → [2.4 会话状态](./4-session-state) → [2.7 会话池模板](./7-fastapi-flask-embed) |
| 只查某个 API 怎么用 | [2.2 构造器完整参数表](./2-constructor-reference) 或 [附录 A · API 参考](/zh/appendix/a-api-reference) |
