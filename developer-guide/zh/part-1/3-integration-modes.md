...
当 embedder 无人值守地驱动 `ACPManager`（CI worker、批处理、队列消费者等）时，把它当作 **headless runtime**。

这里最重要的澄清是：**Headless Runtime 不是第三种集成模式，也不是另一套协议**。它仍然属于上面的 ACP 路线，只是宿主把 `ACPManager` 当成一个无人值守、可轮询状态、可长期运行的运行时来用。

可以直接这样理解：

- **传输没变**：仍然是 ACP 子进程 + stdio + JSON-RPC
- **对象没变**：仍然是 `ACPManager`
- **变的是运行假设**：没有人点确认、没有人回答问题、调用方自己做轮询、门禁和重试

实操时只记 4 条：

- 发 turn 只用 `prompt_once()` 或 `send_prompt()`
- 不要依赖 `send_prompt_nonblocking*`，它们是 internal / unstable
- 同一 server 同一时间只允许一个活跃 turn；撞车就是 `SERVER_BUSY`
- 先看 `get_status()` / `readiness()` 决定能不能发，再看 `last_error` 做诊断

入口支持级别、类型化 `get_status()`、单服务器单活跃 turn 的并发合约、错误分类都冻结在 [`docs/features/headless-runtime.md`](../../../docs/features/headless-runtime.md)；可运行样板是 [`examples/headless_worker.py`](../../../examples/headless_worker.py)。
...
- **宿主是 Python？→ SDK**，除非你需要进程隔离或多租户安全。
- **宿主不是 Python？→ ACP**（唯一选择）。IDE 插件同理。
- **两者可以组合** —— Python 后端用 SDK 跑主流程，再用 `ACPManager` 调其他 ACP server。
- **Headless runtime 不是第三种模式** —— 它就是 `ACPManager` 在无人值守场景下驱动 ACP server。
